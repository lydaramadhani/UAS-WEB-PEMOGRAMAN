# Modul pemesanan tiket khusus peserta event
import datetime
...
# ==========================================
# STAKEHOLDER 1: PESERTA (BUYER / CUSTOMER) VIEWS
# ==========================================
...
def home_view(request):
    ...
    return render(request, 'peserta/event_list.html', context)
...
def event_detail_view(request, slug):
    ...
    return render(request, 'peserta/event_detail.html', context)
...
def checkout_view(request, slug):
    ...
    return render(request, 'peserta/checkout.html', context)
...
def order_confirmation_view(request, order_code):
    ...
    return render(request, 'peserta/order_confirmation.html', context)
...
def ticket_detail_view(request, ticket_code):
    ...
    return render(request, 'peserta/ticket_detail.html', context)


# ==========================================
# STAKEHOLDER 2: EVENT ORGANIZER (EO) VIEWS
# ==========================================
...
# ==========================================
# STAKEHOLDER 3: MASTER ADMIN VIEWS
# ==========================================

@login_required
def admin_user_manage_view(request):
    if not request.user.is_staff:
        messages.error(request, "Akses khusus Master Admin.")
        return redirect('home')

    users = User.objects.select_related('profile').all().order_by('-date_joined')
    return render(request, 'admin/user_manage.html', {'users': users})


@login_required
def admin_user_toggle_role_view(request, user_id):
    if not request.user.is_staff:
        messages.error(request, "Akses khusus Master Admin.")
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        profile.save()
        role_title = "Event Organizer (EO)" if profile.is_organizer else "Peserta (Buyer)"
        messages.success(request, f"Peran pengguna {target_user.username} berhasil diubah menjadi {role_title}.")
